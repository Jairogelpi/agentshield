...
async def get_pending_approvals(identity: VerifiedIdentity = Depends(verify_identity_envelope)):
    ...
    try:
        res = supabase.table("tool_approvals")\
...
async def decide_approval(
    ...
    try:
        ...
        res = supabase.table("tool_approvals").update({
            "status": status,
...
async def create_policy_from_copilot(
    ...
    if identity.role not in ['admin', 'manager', 'owner'] and 'admin' not in (identity.role or '').lower():
        raise HTTPException(403, "Only admins define policies")

    # 1. Resolver ID de herramienta
    # Buscamos por nombre.
    tool_res = supabase.table("tool_definitions").select("id").eq("name", policy.tool_name).eq("tenant_id", identity.tenant_id).execute()
    ...
    if tool_res.data:
        ...
        new_tool = supabase.table("tool_definitions").insert({
    ...
    if policy.target_dept:
        dept_res = supabase.table("departments").select("id").ilike("name", policy.target_dept).execute()
        if dept_res.data:
