...
async def copilot_create_policy(
    ...
    if identity.role not in allowed_roles and 'admin' not in (identity.role or '').lower():
         logger.warning(f"Unauthorized copilot access attempt by {identity.email} (Role: {identity.role})")
...
async def create_policy_from_draft(
    ...
    try:
        ...
        res_tool = supabase.table("tool_definitions").select("id").eq("name", policy['tool_name']).eq("tenant_id", identity.tenant_id).execute()
        ...
        if policy.get('target_dept'):
             ...
             res_dept = supabase.table("departments").select("id").ilike("name", policy['target_dept']).eq("tenant_id", identity.tenant_id).execute()
             if res_dept.data:
