...
async def proxy_embeddings(
    ...
    try:
        ...
        logger.info(
            f"✅ Embeddings generated: {total_tokens} toks (${cost:.6f}) in {duration:.2f}s"
        )
...
async def upload_file(
    ...
    try:
        ...
        await file_guardian.inspect_and_filter(
            ...
            dept_id=getattr(
                identity, "dept_id", None
            ),  # Asumimos que VerifiedIdentity pronto tendrá dept_id
        )
...
@router.get("/v1/files")
async def list_files(identity: VerifiedIdentity = Depends(verify_identity_envelope)):
    """
