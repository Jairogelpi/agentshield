...
async def download_invoice(
    ...
    if not is_admin and identity.role != 'manager':
         raise HTTPException(403, "Access Denied: High-privilege access required for financial documents.")

    # 3. Agregación de Datos Computados
    try:
        invoice_data = await compute_invoice(
            str(identity.tenant_id), cost_center_id, month
        )
