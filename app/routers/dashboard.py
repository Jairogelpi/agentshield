...
async def get_net_cost_impact(tenant_id: str = Depends(get_current_tenant_id)):
    ...
    try:
        ...
        tenant_res = (
            supabase.table("tenants").select("metadata").eq("id", tenant_id).single().execute()
        )
        ...
        roi_percent = (
            ((total_savings - license_cost) / license_cost) * 100 if license_cost > 0 else 0
        )
        ...
        return {
            ...
            "savings": {
                ...
                "arbitrage_contribution": round(
                    total_savings * 0.7, 2
                ),  # Estimated split if column not separated
