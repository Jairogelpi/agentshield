...
async def forget_user(
    target_user_id: str, identity: VerifiedIdentity = Depends(verify_identity_envelope)
):
    """
    ...
    if identity.role not in ["admin", "dpo"]:
        raise HTTPException(
            status_code=403, detail="Privileged Action: Only DPO or Admin can purge user data."
        )

    result = await compliance_officer.execute_right_to_forget(
        str(identity.tenant_id), target_user_id, identity.user_id
    )

    return {"status": "success", "action": "ANONYMIZATION", "certificate": result}


@router.post("/v1/compliance/audit-snapshot")
async def create_audit_snapshot(identity: VerifiedIdentity = Depends(verify_identity_envelope)):
    ...
    result = await compliance_officer.generate_system_snapshot(
        str(identity.tenant_id), identity.user_id
    )

    return {"status": "generated", "certificate": result}


@router.get("/v1/compliance/history")
async def get_compliance_history(identity: VerifiedIdentity = Depends(verify_identity_envelope)):
    """Devuelve el historial de auditoría y links a certificados."""
    res = (
        supabase.table("compliance_actions")
...
async def generate_compliance_report(
    ...
    if identity.role not in ["admin", "dpo", "auditor"]:
        raise HTTPException(
            status_code=403, detail="Only DPO/Admin can generate legal certificates."
        )
    ...
    try:
        pdf_bytes = await compliance_reporter.generate_audit_report(
            str(identity.tenant_id), framework, days
        )
...
async def get_quarantine_queue(identity: VerifiedIdentity = Depends(verify_identity_envelope)):
    ...
    res = (
        supabase.table("quarantine_queue")
        .select("*")
        .eq("tenant_id", identity.tenant_id)
        .eq("status", "PENDING")
        .order("created_at", desc=True)
