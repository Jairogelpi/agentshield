...
async def get_smart_efficiency_factor() -> float:
    ...
    try:
        ...
        ratio_fast = (
            (data.get("hits_1h", 0) / t1h) if t1h > 10 else 0.0
        )  # Mínimo 10 reqs para fiabilidad
        ...
        logger.info("🧠 Smart Efficiency Audit:")
        logger.info(
            f"   - Now (1h): {ratio_fast * 100:.1f}% | Normal (24h): {ratio_slow * 100:.1f}%"
        )
    ...
    try:
        ...
        for m in data:
            try:
                ...
                if p_in <= 0 or p_out <= 0:
                    continue
                if p_in * 1_000_000 > 100:
                    continue  # Ignorar outliers absurdos
                ...
                total_cost_usd = (
                    (1_000_000 * effective_p_in) + (300_000 * p_out)
                ) * volatility_buffer
        ...
        def _save_rules():
            ...
            supabase.table("system_config").upsert(
                {"key": "arbitrage_rules", "value": new_rules, "updated_at": "now()"}
            ).execute()
            # Guardamos caché de modelos
            supabase.table("system_config").upsert(
                {"key": "market_prices", "value": model_prices, "updated_at": "now()"}
            ).execute()
        ...
        logger.info(
            f"✅ Reglas Financieras Recalibradas. {len(model_prices)} modelos indexados para cobro dinámico."
        )
        return new_rules
