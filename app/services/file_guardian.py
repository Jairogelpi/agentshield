...
class FileGuardian:
        ...
        if AI_AVAILABLE:
            try:
                ...
                self.classifier = pipeline(
                    "zero-shot-classification", model="valhalla/distilbart-mnli-12-1", device=-1
                )
    ...
    async def inspect_and_filter(
        self, file: UploadFile, user_id: str, tenant_id: str, dept_id: str | None = None
    ):
        ...
        category, confidence = await self._analyze_content(
            file.filename, content_sample, security_mode
        )
        ...
        if category != "GENERIC":
            ...
            if confidence > 0.90:
                ...
                await self._enforce_policy_block(
                    tenant_id, dept_id, user_id, file.filename, category, reason, security_mode
                )

            # 🟡 AMARILLO: Zona de Duda (40% - 90%) -> CUARENTENA
            elif confidence > 0.40:
                logger.warning(f"⚠️ Ambiguous Content ({confidence:.2f}). Sending to Quarantine.")
                await self._send_to_quarantine(
                    tenant_id, user_id, file.filename, file_hash, category, confidence
                )
    ...
    async def _is_whitelisted(self, file_hash: str, tenant_id: str) -> bool:
        ...
        try:
            res = (
                supabase.table("semantic_whitelist")
                .select("file_hash")
    ...
    async def _send_to_quarantine(
        self, tenant_id, user_id, filename, file_hash, category, confidence
    ):
        ...
        try:
            ...
            await trigger_webhook(
                tenant_id,
                "security.quarantine",
                {"filename": filename, "confidence": confidence, "category": category},
            )
        except Exception as e:
    ...
    def _semantic_verify_local(self, filename: str, text: str, category: str) -> tuple[str, float]:
        ...
        if category == "INVOICE":
            ...
            target_label = "sensitive content"
    ...
    async def _semantic_verify_openai(
        self, filename: str, text: str, category: str
    ) -> tuple[str, float]:
    ...
    async def _enforce_policy_block(
        self, tenant_id, dept_id, user_id, filename, category, reason, mode
    ):
            ...
            for policy in policies:
                ...
                if category in rules.get("block_categories", []):
                    await self._audit_block(
                        tenant_id, policy["id"], user_id, filename, category, reason, mode
                    )
