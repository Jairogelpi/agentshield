...
def evaluate_logic(rule: dict, context: PolicyContext) -> bool:
    ...
    if "var" in rule:
        ...
        ctx_val = None
        if var_name == "cost_usd":
            ctx_val = context.estimated_cost
        elif var_name == "model":
            ctx_val = context.model
        elif var_name == "intent":
            ctx_val = context.intent
        ...
        if ctx_val is not None:
            ...
            if op == "==" and ctx_val == val:
                return True
            if op == "in" and val in ctx_val:
                return True  # "gpt" in "gpt-4"
    ...
    for p in policies:
        ...
        if is_hit:
            ...
            if p["mode"] == "SHADOW":
                ...
                logger.info(
                    f"👻 SHADOW HIT: Policy '{p['name']}' triggered for {context.user_email}"
                )
