...
class AgentShieldRLArbitrator:
    ...
    async def update_learning(self, state: str, action_model: str, reward: float):
        ...
        # Log learning
        logger.info(
            f"🧠 RL Update [{state}][{action_model}]: {current_q:.4f} -> {new_q:.4f} (Reward: {reward:.4f})"
        )

    def calculate_reward(
        self, cost_saved: float, rerank_score: float, latency_ms: float, user_satisfaction=1.0
    ) -> float:
        """
    ...
    async def find_best_bidder(
        self,
        requested_model: str,
        analysis: dict,
        max_output_tokens: int = 1000,
        tenant_allowlist: list = None,
    ):
        """
        ...
        for m in market_models:
            ...
            if tenant_allowlist and m_id not in tenant_allowlist:
                continue
            if context < (input_tokens + max_output_tokens):
                continue
            candidates.append(m)
        ...
        savings = (
            (target_price - float(winner.get("price_out", 0))) / target_price
            if target_price > 0
            else 0
        )
    ...
    async def get_potential_arbitrage_gain(
        self, original_model: str, prompt_complexity_score: float
    ) -> tuple[float, str]:
...
async def get_best_provider(
    target_quality: str, max_latency_ms: int = 2000, messages: list = [], input_tokens: int = 0
) -> dict:
    """
    ...
    try:
        ...
        winner_id, reason, savings = await arbitrage_engine.find_best_bidder(
            requested_model=target_quality, analysis=analysis
        )
        ...
        if winner_id != target_quality:
            ...
            if winner_obj:
                logger.info(
                    f"💰 Arbitrage Opportunity: {target_quality} -> {winner_id} (Reason: {reason}, Est. Savings: {savings * 100:.1f}%)"
                )
                return {
                    "model": winner_id,
                    "api_base": winner_obj.get(
                        "api_base"
                    ),  # Might be None if it's a standard cloud provider
                    "provider": winner_obj.get("provider", "openai"),
                    "reason": reason,
                    "rl_state": analysis.get(
                        "rl_state"
                    ),  # <--- FIX: Retornar estado para feedback loop
                }
