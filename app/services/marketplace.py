...
class MarketplaceService:
    ...
    async def enforce_data_license(self, identity: VerifiedIdentity, documents: List[Dict]) -> List[Dict]:
        """
        ...
        collection_ids = list(set(d.get('collection_id') for d in documents if d.get('collection_id')))
        ...
        for doc in documents:
            ...
            # 2. ACCESO DENEGADO
            if not rights:
                logger.warning(f"🚫 Access Denied: User {identity.email} tried to access collection {col_id} without paying.")
    ...
    async def _get_access_rights(self, identity, collection_ids):
        ...
        try:
             res = supabase.rpc("check_marketplace_access", {
    ...
    async def _summarize_content(self, text):
        ...
        messages = [{"role": "system", "content": "Summarize strictly. Do not quote."}, {"role": "user", "content": text}]
        try:
