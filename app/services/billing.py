...
from app.db import increment_spend, redis_client, supabase
...
async def settle_knowledge_exchange(buyer_tid: str, seller_tid: str, original_model_cost: float):
    ...
    with tracer.start_as_current_span("sovereign_settlement") as span:
        try:
            ...
            span.set_attribute(
                "knowledge.buyer_savings", float(original_model_cost - float(discounted_price))
            )
            ...
            await increment_spend(
                ...
                "default",
                discounted_price,
                {"mode": "KNOWLEDGE_PURCHASE", "source_tenant": seller_tid},
            )
            ...
            await increment_spend(
                ...
                "default",
                -seller_revenue,
                {"mode": "KNOWLEDGE_SALE", "target_tenant": buyer_tid},
            )
    ...
    with tracer.start_as_current_span("budget_integrity_check") as span:
        try:
            ...
            if cur + estimated_cost > lim:
                ...
                logger.warning(
                    f"⛔ TITANIC PROTOCOL: Tenant {tenant_id} blocked. Budget Exceeded ({cur}/{lim})."
                )
            ...
            if current_velocity > PANIC_THRESHOLD:
                ...
                logger.critical(
                    f"🔥 KILL SWITCH: Tenant {tenant_id} spending ${current_velocity}/min! Freezing account."
                )
                # Auto-Freeze (podríamos poner una flag en Redis)
                await redis_client.setex(
                    f"security:freeze:{tenant_id}", 300, "1"
                )  # Freeze por 5 min
