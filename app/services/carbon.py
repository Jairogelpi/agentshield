...
class CarbonGovernor:
    async def get_dynamic_config(self) -> dict:
        ...
        try:
            ...
            res = supabase.table("system_config").select("value").eq("key", "energy_per_1k_tokens").maybe_single().execute()
            if res.data:
                val = res.data['value']
                await redis_client.setex("system_config:energy_per_1k_tokens", 3600, json.dumps(val))
                return val
        except:
            pass
        return {"default": 0.001}

    async def estimate_footprint(self, model: str, prompt_tokens: int, output_tokens: int = 0) -> float:
    ...
    async def check_budget_and_route(self, ctx: DecisionContext) -> DecisionContext:
        ...
        if ctx.intent in green_intents and is_heavy_model:
            ...
            ctx.log("CARBON", f"Green Routing LIVE: Intensity is {intensity} gCO2/kWh. Routed to Eco.")
            return ctx

        # C. Chequeo de Presupuesto Real (desde Tabla departments)
        if ctx.dept_id:
            res = supabase.table("departments").select("co2_monthly_limit_grams, current_co2_spend_grams").eq("id", ctx.dept_id).maybe_single().execute()
            if res.data:
                ...
                if current > limit:
                    if not ctx.green_routing_active:
                        ctx.effective_model = "agentshield-eco"
                        ctx.log("CARBON", f"DEPT BUDGET EXCEEDED ({current}/{limit}g). Forcing Eco Mode.")
    ...
    async def log_emission(self, tenant_id, dept_id, user_id, trace_id, model, grams, avoided_grams=0.0):
        ...
        try:
            ...
            if dept_id:
                supabase.rpc("increment_dept_carbon", {
                    "p_dept_id": dept_id,
                    "p_grams": grams
                }).execute()
