...
class ForensicService:
    async def reconstruct_timeline(self, tenant_id: str, trace_id: str):
        ...
        try:
            ...
            if receipt.data:
                timeline.append(
                    {"ts": receipt.data[0]["created_at"], "type": "FINISH", "data": receipt.data[0]}
                )
        ...
        try:
            policies = (
                ...
                .select("*")
                .eq("tenant_id", tenant_id)
                .contains("metadata", {"trace_id": trace_id})
                .execute()
            )
        ...
        try:
            tools = (
                ...
                .select("*")
                .eq("tenant_id", tenant_id)
                .contains("tool_arguments", {"_trace_id": trace_id})
                .execute()
            )
            for t in tools.data:
        ...
        try:
            ...
            if security.data:
                timeline.append(
                    {
                        "ts": security.data[0]["created_at"],
                        "type": "SECURITY_ALERT",
                        "data": security.data[0],
                    }
                )
        except Exception as e:
    ...
    def generate_legal_pdf(self, timeline: list, trace_id: str) -> bytes:
        ...
        pdf.cell(
            0, 10, f"Generated: {datetime.datetime.now()}", align="C", new_x="LMARGIN", new_y="NEXT"
        )
        pdf.ln(10)
        ...
        pdf.multi_cell(
            0,
            5,
            "CONFIDENTIAL: This document contains a cryptographic reconstruction of an AI interaction. It is intended for internal auditing and legal compliance verification.",
        )
        pdf.ln(5)
        ...
        for step in timeline:
            ...
            if "embedding" in data_clean:
                del data_clean["embedding"]  # No imprimir vectores
        ...
        pdf.cell(
            0,
            10,
            "Digital Signature: SHA-256 Verified via AgentShield Immutable Ledger.",
            align="C",
            new_x="LMARGIN",
            new_y="NEXT",
        )
