...
class SemanticRouter:
    async def classify_intent(self, tenant_id: str, prompt: str) -> str:
        ...
        try:
            res = supabase.table("intent_definitions").select("name, description").eq("tenant_id", tenant_id).execute()
            intents = res.data or []
    ...
    async def check_semantic_budget(self, identity, intent: str):
        ...
        try:
            ...
            res_intent = supabase.table("intent_definitions")\
                .select("id")\
                .eq("name", intent)\
            ...
            res_budget = supabase.table("semantic_budgets")\
                .select("*")\
            ...
            if not budget_rule:
                ...
                res_global = supabase.table("semantic_budgets")\
                    .select("*")\
                    .eq("intent_id", intent_id) \
            ...
            if action == 'BLOCK':
                return False, 0.0, f"Department '{identity.dept_id}' is BLOCKED from '{intent}' tasks."
